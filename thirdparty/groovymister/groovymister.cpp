...
int GroovyMister::CmdSwitchres(double pClock, uint16_t hActive, uint16_t hBegin, uint16_t hEnd, uint16_t hTotal, uint16_t vActive, uint16_t vBegin, uint16_t vEnd, uint16_t vTotal, uint8_t interlace)
{
	...
	// Round once, at the end. Rounding the line to a whole microsecond before
	// scaling it discards most of the precision: an 808-pixel line at
	// 25.426547 MHz is 31.7776 us, which became 32.0 and made a 525-line
	// frame 0.7% too long. Multiplying first keeps the same units and the
	// same integer arithmetic everywhere else, and leaves 0.07%.
	m_widthTime = round(10.0 * (double) hTotal / pClock); //tenths of a microsecond, time to raster 1 line
...
{  
	...
	if (USE_RIO)
	{
		...
		if (::GetQueuedCompletionStatus(m_hIOCP, &numberOfBytes, &completionKey, &pOverlapped, dwMilliseconds))
		{
			...
			observe(GM_RECEIVE_FAILED, nullptr, 0, WSAGetLastError());
			}
		}
...
void GroovyMister::WaitSync(void)
{
	...
	if (USE_RIO)
	{
		...
		uint64_t nowMs = m_verbose >= 1 ? monotonicMs() : 0;
...
void GroovyMister::PollInputs(void)
{
	...
	do
	{
		...
		if (len == 9 || len == 17 || len == 13 || len == 25) //blit joystick digital or analog (v1/v2)
		{
			...
			bool accepted = joyFrame > joyInputs.joyFrame ||
			    (joyFrame == joyInputs.joyFrame && joyOrder > joyInputs.joyOrder);
			observe(accepted ? GM_INPUT_ACCEPTED : GM_INPUT_STALE,
			        m_bufferInputsReceive, len);
			if (accepted)
			{
				setFpgaJoystick(len);
				observe(GM_INPUT_DECODED, &joyInputs, sizeof(joyInputs));
			}
		}
		else if (len >= 0 && len != 37 && len != 41)
		{
			observe(GM_INPUT_OTHER, m_bufferInputsReceive, len);
		}
...
#endif
	if (sendto(m_sockFD, (char *) cmd, cmdSize, 0, (struct sockaddr *)&m_serverAddr, sizeof(m_serverAddr)) < 0)
	{
#ifdef _WIN32
		observe(GM_SEND_FAILED, nullptr, 0, WSAGetLastError());
#else
		observe(GM_SEND_FAILED, nullptr, 0, errno);
#endif
	}
}
...
{
	observe(GM_ACK, m_bufferReceive, sizeof(m_bufferReceive));
	uint8_t fpgaBits;
