...
// Optional synchronous observations on the calling thread. Packet storage is
// valid only for the callback; observers must not call back into the client.
enum gm_observation {
    GM_INPUT_ACCEPTED, GM_INPUT_STALE, GM_INPUT_OTHER,
    GM_ACK, GM_SEND_FAILED, GM_COMPLETION_FAILED, GM_RECEIVE_FAILED,
    GM_INPUT_DECODED
};
typedef void (*gm_observer_fn)(void *opaque, gm_observation event,
                               const void *data, int size, int status);
...
class GroovyMister
{
 public:
	...
	void setObserver(gm_observer_fn observer, void *opaque) {
		m_observer = observer;
		m_observerOpaque = opaque;
	}
 ...
 private:
	...
	uint8_t m_verbose;
	gm_observer_fn m_observer = nullptr;
	void *m_observerOpaque = nullptr;
	void observe(gm_observation event, const void *data = nullptr,
	             int size = 0, int status = 0) {
		if (m_observer) m_observer(m_observerOpaque, event, data, size, status);
	}
